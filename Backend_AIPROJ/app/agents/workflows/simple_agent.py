...
from app.tools.summarizer import summarize_document_tool, get_summarizer_tool_definition

logger = setup_logger()

# Summarization intent signals (high priority - checked BEFORE general signals)
SUMMARIZATION_SIGNALS = [
    "summarize", "summary", "overview document", "document overview", "what does this doc say",
    "key points", "main ideas", "give me a summary", "brief overview",
    "what's in the document", "tldr", "tl;dr",
    "main points", "highlights", "condensed version", "explain document"
]
...
def classify_intent(prompt: str) -> str:
    """
    Classify if a prompt is summarization, document-query, or general using context-aware heuristics.
    
    Strategy:
    1. Check for summarization signals (highest priority - new tool routing)
    2. Check for explicit document references (high confidence)
    3. Check for general/reasoning question patterns (negative signals)
    4. Check for semantic patterns that might indicate document queries
    5. Default to general to avoid false positives
    
    Args:
        prompt: The user's prompt
    
    Returns:
        str: "summarization" if summary request, "document_query" if document-related, "general_query" otherwise
    """
    prompt_lower = prompt.lower()
    
    # Step 0: Check for summarization signals (HIGHEST PRIORITY)
    for signal in SUMMARIZATION_SIGNALS:
        if signal in prompt_lower:
            logger.info(f"intent=summarization - Summary signal matched: '{signal}'")
...
class SimpleAgent:
    ...
    def run(self, prompt: str, max_iterations: int = 5, verbose: bool = False) -> Dict[str, Any]:
        ...
        # If summarization request, force summarize_document tool immediately
        if intent == "summarization":
            logger.info("routing=force_summarize_tool - Summarization request detected, calling summarize_document directly")
            
            # Update agent metadata for decision transparency
            agent_meta["decision_route"] = "force_summarize_document"
            agent_meta["reason"] = "Summarization request detected via intent classification"
            agent_meta["tool_selected"] = "summarize_document"
            agent_meta["tool_confidence"] = 0.95
            agent_meta["alternatives_considered"] = ["ask_document", "summarize_document"]
            
            # Legacy fields
            agent_meta["routing"] = "force_summarize_tool"
            agent_meta["tool_used"] = "summarize_document"
            agent_meta["mode"] = "summarize_direct"
            ...
            try:
                ...
                # Execute summarize_document tool directly - returns tuple with telemetry
                tool_result, tool_telemetry = self._execute_tool("summarize_document", {})
                ...
                # Generate user message based on summarization mode
                mode_used = tool_telemetry.get("mode_used", "unknown")
                chunks_used = tool_telemetry.get("chunks_used", 0)
                
                if chunks_used == 0:
                    user_message = "No relevant content found for summarization."
                elif mode_used == "extractive":
                    user_message = f"Summary generated using RAG-only extractive mode from {chunks_used} document chunks."
                elif mode_used == "hybrid":
                    user_message = f"Summary generated using intelligent hybrid mode (RAG + LLM synthesis) from {chunks_used} document chunks."
                else:
                    user_message = "Summary generated successfully."
                
                logger.info(f"user_message=summarization - mode={mode_used} chunks={chunks_used}")
                ...
                error_msg = f"Summarization tool failed: {str(e)}"
                logger.error(error_msg)
                tool_result = error_msg
                user_message = "Failed to generate summary due to an error."
                agent_meta["failure_reason"] = str(e)
                agent_meta["error_class"] = type(e).__name__
            ...
            # Build response with proper transparency
            response = {
                "answer": tool_result,
            ...
            # Include agent_meta only if verbose=True
            if verbose:
                response["agent_meta"] = agent_meta
                logger.info(f"verbose=true - Including full agent_meta in response")
            
            logger.info(f"Summarization complete - latency={total_latency}ms iterations=1")
            return response
        ...
        if intent == "document_query":
            ...
            try:
                ...
                # Determine user message based on new semantic fields (confidence_path + answer_status)
                confidence_path = tool_telemetry.get("confidence_path", "unknown")
                answer_status = tool_telemetry.get("answer_status", "unknown")
                confidence = tool_telemetry.get("confidence_top", 0)
                mode = tool_telemetry.get("mode", "unknown")
                
                # Priority 1: Use user_message from RAG layer if provided (for uncertain/no-answer cases)
                if "user_message" in tool_telemetry and tool_telemetry["user_message"]:
                    user_message = tool_telemetry["user_message"]
                    logger.info(f"user_message=from_rag - Using RAG-provided message: {user_message}")
                # Priority 2: Generate message based on semantic fields
                elif answer_status == "confident_answer":
                    if confidence_path == "high_confidence_direct":
                        user_message = "Answer generated successfully from document content with high confidence."
                        logger.info(f"user_message=confident - confidence_path={confidence_path} answer_status={answer_status}")
                    else:
                        user_message = "Answer generated successfully from document content."
                        logger.info(f"user_message=confident_other - confidence_path={confidence_path} answer_status={answer_status}")
                elif answer_status == "uncertain_answer":
                    user_message = f"This answer is based on weak matches (confidence: {confidence:.2f}). The document may not clearly contain this information."
                    logger.info(f"user_message=uncertain - confidence_path={confidence_path} answer_status={answer_status}")
                elif answer_status == "no_answer_in_document":
                    user_message = "The document does not appear to contain information related to your question."
                    logger.info(f"user_message=no_answer - confidence_path={confidence_path} answer_status={answer_status}")
                elif answer_status == "not_enough_confidence_to_answer":
                    user_message = "I couldn't find enough relevant information in the document to confidently answer your question."
                    logger.info(f"user_message=insufficient_confidence - confidence_path={confidence_path} answer_status={answer_status}")
                # Fallback to legacy mode-based logic
                elif mode == "rag_confident":
                    user_message = "Answer generated successfully from document content."
                    logger.info("user_message=success_confident - High confidence RAG answer (legacy mode)")
                elif mode == "rag_llm_hybrid":
                    user_message = f"Document evidence found (confidence: {confidence:.2f}), enhanced with reasoning."
                    logger.info("user_message=success_hybrid - Low confidence, used LLM enhancement (legacy mode)")
                elif mode == "safe_fallback":
                    user_message = "Unable to find relevant information in the document. Providing safe response."
                    logger.info("user_message=no_content - Document does not contain requested information (legacy mode)")
                else:
                ...
                logger.info(f"Agent completed via forced RAG routing mode={agent_meta.get('mode')} confidence_path={agent_meta.get('confidence_path')} answer_status={agent_meta.get('answer_status')} cache_hit={agent_meta.get('cache_hit')}")
                logger.info(f"decision_transparency - latency={total_latency}ms tool_latency={tool_latency}ms confidence_path={agent_meta.get('confidence_path')} answer_status={agent_meta.get('answer_status')}")
                ...
                if verbose:
                    ...
                    # Minimal meta for backward compatibility, now with semantic transparency
                    response["meta"] = {
                        "mode": agent_meta.get("mode"),
                        "confidence_path": agent_meta.get("confidence_path"),
                        "answer_status": agent_meta.get("answer_status"),
                        "retrieval_score_primary": agent_meta.get("retrieval_score_primary"),
...
For general questions not requiring tools, provide helpful direct responses."""
        ...
        try:
            ...
            # Ensure complete telemetry before including in response
            from app.core.telemetry import ensure_complete_telemetry
            agent_meta = ensure_complete_telemetry(agent_meta)
            ...
            # Ensure complete telemetry before including in error response
            from app.core.telemetry import ensure_complete_telemetry
            agent_meta = ensure_complete_telemetry(agent_meta)
            agent_meta["fallback_triggered"] = True
            agent_meta["degradation_level"] = "failed"
